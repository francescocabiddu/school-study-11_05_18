...
libs <- c("dplyr", "magrittr", "tidyr", "stringr", "knitr", 'ggplot2')
...
ggplot(span_size_nw_raw, aes(x=age_month, y=span_size, shape = list))+
  geom_jitter(height = 0.25, width = 0.25) +
  ggtitle('Span size by age') +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(x = "Age in months", y = "Span size", shape = "List") +
  scale_shape_manual(labels = c("Digits", "Words"), values = c(0, 3)) 
```
**Figure 1.** Span size by age for digit and word lists (25% of random noise was added at each y axis tick value to avoid overplotting)

<br>

```{r echo=FALSE, fig.width=7, fig.height=5}
ggplot(span_tot_nw_raw, aes(x=age_month, y=span_tot, shape = list))+
  geom_jitter(height = 0.25, width = 0.25) +
...
```
**Figure 2.** Span tot by age for digit and word lists (25% of random noise was added at each y axis tick value to avoid overplotting)
